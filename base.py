...
def main(seed, epochs, lr, batch_size, valid_batch_size, valid_interval, valid_thr,
    ...
    try:
        ...
        model = smp.UnetPlusPlus(encoder_name=encoder_name, encoder_weights=encoder_weights, in_channels=3, classes=len(CLASSES))
        print("Model Created.")

        train_tf = A.Resize(2048, 2048)
        valid_tf = A.Resize(2048, 2048)
            ...
            if (epoch + 1) % valid_interval == 0:
                ...
                with torch.no_grad():
                    for step, (images, masks) in tqdm(enumerate(valid_loader), total=len(valid_loader)):
                        ...
                        with amp.autocast():  # Mixed precision inference
                            outputs = model(images)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--clahe', type=bool, default=True, help='clahe augmentation')
    parser.add_argument('--cp', type=bool, default=True, help='copypaste augmentation')
